...
canceled=InlineKeyboardMarkup([[InlineKeyboardButton("🍄 ألغيت 🍄", callback_data="canceled")]])
completed=InlineKeyboardMarkup([[InlineKeyboardButton("😎 اكتمل 😎", callback_data="completed")]])
...
async def notInPROCESS(chat_id, message, current, total, deleteID):
    if chat_id in PROCESS:
        ...
        await message.edit(
            text=f"`تم الإلغاء في {current}/{total} صفحات..` 🙄",
        ...
        if nabilanavab==False:
            ...
            if data in ["IA", "DA", "IR", "DR"]:
                ...
                for i in range(0, len(totalPgList), 10):
                    ...
                    if callbackQuery.message.chat.id in PROCESS:
                        await downloadMessage.edit(
                            text=f"`تحميل: {cnvrtpg}/{int(pageStartAndEnd[1])+1 - int(pageStartAndEnd[0])} صفحات.. 🐬`",
                            reply_markup=cancel
                        )
                    else:
                        shutil.rmtree(f'{callbackQuery.message.message_id}')
            ...
            if data in ["IS", "DS"]:
                ...
                for i in range(0, len(totalPgList), 10):
                    ...
                    for pageNo in pgList:
                        ...
                        if cnvrtpg%5==0:
                            await downloadMessage.edit(
                                text=f"`تم تحويل: {cnvrtpg}/{len(totalPgList)} صفحات.. 🤞`",
                                reply_markup=cancel
                            )
                            if await notInPROCESS(
                    ...
                    if await notInPROCESS(
                        ...
                        return
                    await downloadMessage.edit(
                        text=f"`تحميل: {cnvrtpg}/{len(totalPgList)} صفحات.. 🐬`",
                        reply_markup=cancel
                    )
                    if data=="IS":
                        if callbackQuery.message.chat.id not in PROCESS:
...
async def _KEXTRACT(bot, callbackQuery):
    try:
        ...
        if nabilanavab==False:
            downloadMessage=await callbackQuery.message.reply_text(
                text="`تنزيل ملف pdf الخاص بك ..` ⏳", quote=True
            )
            file_id=callbackQuery.message.reply_to_message.document.file_id
            fileSize=callbackQuery.message.reply_to_message.document.file_size
            # DOWNLOAD PROGRESS
            ...
            if data in ["KIA", "KDA", "KIR", "KDR"]:
                ...
                for i in range(0, len(totalPgList), 10):
                    ...
                    for pageNo in pgList:
                        ...
                        if cnvrtpg%5==0:
                            await downloadMessage.edit(
                                text=f"`تم تحويل: {cnvrtpg}/{int(pageStartAndEnd[1])+1 - int(pageStartAndEnd[0])} صفحات.. 🤞`",
                                reply_markup=cancel
                            )
                        if callbackQuery.message.chat.id not in PROCESS:
                            try:
