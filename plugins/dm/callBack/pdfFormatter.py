    ...
    try:
        ...
        if data[0]=="K":
            ...
            if int(number_of_pages) >= 5:
                await callbackQuery.answer("أرسل لي ملف أقل من 5 صفحات ..🏃")
                return
        # ADD TO PROCESS
        ...
        if downloadLoc is None:
            ...
            return
        await downloadMessage.edit("`بدأ التنسيق ..` 📖")
        ...
        if data=="format":
            ...
            if not(checked=="pass"):
                ...
                return
        unFormated=f'{callbackQuery.message.message_id}/غير منسق.jpeg'
        # OPEN INPUT PDF
        r=fitz.Rect(0,0,0,0)
            ...
            with fitz.open() as outPDF:
                ...
                if nOfPages > 5:
                    await downloadMessage.edit(
                        text="__بالنسبة لبعض الأسباب ، يدعم تنسيق A4 ملفات PDF التي تحتوي على أقل من 5 صفحات__"
                             f"\n\nعدد الصفحات: {nOfPages} ⭐"
                    )
                    PROCESS.remove(callbackQuery.message.chat.id)
        ...
        await callbackQuery.message.reply_document(
            ...
            thumb=PDF_THUMBNAIL, caption="ملف PDF منسق (A4)"
        )
        await downloadMessage.delete()
        PROCESS.remove(callbackQuery.message.chat.id)
        shutil.rmtree(f"{callbackQuery.message.message_id}")
        ...
        try:
            print("FormatToA4: " , e)
            await downloadMessage.edit(f"خطا: `{e}`")
            shutil.rmtree(f"{callbackQuery.message.message_id}")
            PROCESS.remove(callbackQuery.message.chat.id)
        except Exception:
            pass
